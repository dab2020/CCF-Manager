...
def generatecustomerinvoice(name, phone, product, total, paymeth, vetflag, depflag, uniqueid, zipcode, address1, otherin):
    if vetflag:
        totalwtax = 1.2 * int(total)
        vet = 0.2 * int(total)
        vet = round(vet, 2)
    ...
    doc.render(
        {"name": name, "invoice_list": product, "phone": phone, "total": total, "date": today, "paymeth": paymeth,
         ...
         "add": address1, "otherinfo": otherin})
    doc.save(filename)
    cont2pdf(filename)
    deldocx(filename)
    printf(filepdf)
